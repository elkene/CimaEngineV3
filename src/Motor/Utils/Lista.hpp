...
#include "Nodo.hpp"
namespace CE {
    template <typename T> class Lista : public Iterador <T>, public IManipularDatos <T> {
        public:
        explicit Lista()
    ...
    template <typename T> void Lista <T>::agregarDato(const T& d) {
        ...
        //make_shared, no te lo permite pero
        //lo que podemos hacer es instanciar un shared_ptr
        //con un rawpointer
